    ...
    if request.method == 'POST':
        ...
        if invoice_form.is_valid() and all(f.is_valid() for f in forms):
            ...
            chosen_invoice.invoice_doc = SimpleUploadedFile(
                f'{selected_business_account.name} Invoice-{chosen_invoice.invoice_id}.pdf', pdf_file,
...
def generate_pdf_invoice(request, id):
    ...
    selected_invoice.invoice_doc = SimpleUploadedFile(
        selected_business_account.name + selected_invoice.invoice_id + '.pdf', pdf, content_type='application/pdf')
    selected_invoice.save()
    ################################################

    # Create a response object and specify the PDF content type
    response = HttpResponse(pdf, content_type='application/pdf')
    response['Content-Disposition'] = 'attachment; filename='+selected_business_account.name+'-Invoice-'+selected_invoice.invoice_id + '.pdf'
...
def invoice_verification(request, id):
    ...
    if Invoice.objects.filter(invoice_id=id):
        ...
        messages.success(request, f'Verification Successful. This invoice was generated by {invoice_id.business_account.name}')
        return render(request, 'invoice/invoice_confirmation.html', context)
    else:
        context = {
            'invoice_id': "",
        }
        messages.warning(request, f'Verification Failed. This invoice was NOT generated by {invoice_id.business_account.name}')
