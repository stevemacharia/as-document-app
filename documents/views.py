...
def generate_pdf_quotation(request, id):
    ...
    selected_quotation.quotation_doc = SimpleUploadedFile(
        selected_business_account.name + selected_quotation.quotation_id + '.pdf', pdf, content_type='application/pdf')
    selected_quotation.save()
    ################################################
...
def quotation_verification(request, id):
    ...
    if Quotation.objects.filter(quotation_id=id):
        ...
        messages.success(request, f'Verification Successful. This Quotation was generated by { selected_quotation.business_account }')
        return render(request,  'documents/quotation_confirmation.html', context)
    else:
        context = {
            'document_id': "",
        }
        messages.warning(request, f'Verification Failed. This Quotation was NOT generated by { selected_quotation.business_account }')
        return render(request, 'documents/quotation_confirmation.html', context)
