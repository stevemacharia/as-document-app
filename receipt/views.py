...
def generate_pdf_receipt(request, id):
    ...
    selected_receipt.receipt_doc = SimpleUploadedFile(
        selected_business_account.name + selected_receipt.receipt_id + '.pdf', pdf, content_type='application/pdf')
    selected_receipt.save()
    ################################################
...
def receipt_verification(request, id):
    ...
    if Receipt.objects.filter(receipt_id=id):
        ...
        messages.success(request, f'Verification Successful. This Receipt was generated by { selected_receipt.business_account }')
        return render(request, 'receipt/receipt_confirmation.html', context)
    else:
        context = {
            'receipt_id': "",
        }
        messages.warning(request, f'Verification Failed. This Receipt was NOT generated by { receipt.business_account }')
        return render(request, 'receipt/receipt_confirmation.html', context)
